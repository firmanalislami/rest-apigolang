...
var db, err = sql.Open("mysql", "root:localgw@tcp(127.0.0.1:3306)/stockbarang")
...
func getAll(c *gin.Context) {
	...
	rows, err := db.Query("select id, username, first_name, password, middle_name, last_name, email, mobile_phone, login_attempt, active_status from user;")
	...
	for rows.Next() {
		rows.Scan(&user.Id, &user.Username, &user.FirstName, &user.Password, &user.MiddleName, &user.LastName, &user.Email, &user.MobilePhone, &user.LoginAttempt, &user.ActiveStatus)
...
func update(c *gin.Context) {
	...
	if Username == "" || Password == "" || OldPassword == "" || FirstName == "" || Email == "" {
		c.JSON(http.StatusOK, gin.H{
			"message": fmt.Sprintf("Please all Required field"),
		})
		return
	}
